...
namespace jerseyShoppingCartMvcUI.Repositories
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<int> AddItem(int jerseyId, int qty)
        {
            ...
            try
            {
                ...
                var cartItem = _db.CartDetails
                                  .FirstOrDefault(a => a.ShoppingCartId == cart.Id && a.jerseyId == jerseyId);
                ...
                {
                    var jersey = _db.jerseys.Find(jerseyId);
                    cartItem = new CartDetail
                    {
                        jerseyId = jerseyId,
                        ShoppingCartId = cart.Id,
                        Quantity = qty,
                        UnitPrice = jersey.Price  // it is a new line after update
                    };
        ...
        public async Task<int> RemoveItem(int jerseyId)
        {
            //using var transaction = _db.Database.BeginTransaction();
            string userId = GetUserId();
            try
        ...
        public async Task<bool> DoCheckout(CheckoutModel model)
        {
            ...
            try
            {
                ...
                foreach(var item in cartDetail)
                {
                    ...
                    var stock = await _db.Stocks.FirstOrDefaultAsync(a => a.jerseyId == item.jerseyId);
                    if (stock == null)
                    {
